...
- **FR2.7** — AI-assisted content analysis is a capability of the system, not something a student configures: the backend holds one Anthropic key of its own, never returned by any endpoint and never logged. When it is set, uploaded material is read by Claude to identify topics, *estimate the study time each one needs* (replacing the fixed default durations), and — in the analysis flows of FR2.8 and FR2.10 — summarise the material and list its key points. When no key is configured, or the call fails for any reason, the keyword heuristic runs instead — an AI outage must never block an upload. **The AI only understands unstructured content. Every decision that follows — which topic a file belongs to, priorities, mastery weighting, time allocation, scheduling — is LearnSprint's own deterministic, explainable logic** (see [ADR 0013](docs/adr/0013-content-intelligence-pipeline.md)).
- **FR2.2** — Full editing of the extracted topic list: add, delete, rename, and adjust time estimates by hand when the extracted default doesn't match reality — either per learning action, or for the whole topic, in which case the new total is split across its actions in proportion to their current estimates.
- **FR2.3** — A topic is the unit of work — the "task" on the board — and its learning actions are its subtasks. Each new topic automatically gets the three default actions read, summarize, quiz; the student can add further subtasks, rename any subtask, and delete subtasks (a topic keeps at least one). Deleting a subtask removes every group member's progress on it. A topic also carries an optional description, and shows the student as its assignee (there is no collaboration on a single topic — see [ADR 0012](docs/adr/0012-topic-as-the-task.md)).
...
- **FR2.6** — Let the user set a topic's priority by hand: low, medium (the default) or high. It is never extracted or inferred by the system — the extraction step (FR2.1) only pulls topic names, nothing about importance. A high-priority topic is the "core exam topic" the scheduler favours (FR3.2); low and medium are treated alike by the scheduler for now.
- **FR2.8** — In addition to the initial batch upload (FR2.1), a student may upload a single additional file to a course at any time. The system stores it (FR2.9) and treats the file as **one academic unit — one topic**, never one per heading or slide: it extracts a title, a summary, one to five key points (the unit's subtopics) and an estimated study time — by AI or, without it, by the heading heuristic in the same one-file-one-topic shape — and LearnSprint's deterministic matcher ranks that content against the course's existing topics, returning a recommendation (attach to an existing topic, or create a new one) with a confidence, a reason in plain words, and up to two ranked alternatives. The system never assigns automatically: the student confirms, choosing the recommended topic, another topic, or a new topic with the suggested title editable. Confirmation is idempotent — a repeated confirmation never creates a second topic or attaches a file twice.
- **FR2.9** — A topic holds the student's original materials: files uploaded to that topic are kept as they are, listed with the topic, downloadable, and deletable by the student who uploaded them. Materials arrive either directly (attached in the topic view) or through analysis (FR2.8, FR2.10), which stores the file first and files it under the chosen topic on confirmation. Filing analysed material under an existing topic refreshes that topic's description from the material's summary and key points; the student's status, mastery rating and completed subtasks are never touched. Materials are private to their uploader — not shared with a Study Group in this release.
- **FR2.10** — When an uploaded document has an identifiable lecture/week structure (a syllabus), the student may run structure analysis instead of the direct batch upload (FR2.1). The uploaded file is stored immediately, like any material. The system proposes roughly one Topic per lecture — not one per subtopic — each with a summary, key points and an estimated study time, cross-checked against the course's existing topics by the same matcher as FR2.8; the student reviews and edits the proposed list (rename, drop, or keep a proposal as a match to an existing topic) before anything is created. No proposed Topics are persisted until confirmation, and confirming twice never creates duplicates.

### 3. Scheduling & learning strategy
- **FR3.1** — Build a schedule based on the user's constraints and the exam date, allocating time to each learning action. The schedule is **one plan across all of the student's courses**: courses are scheduled in order of exam date, earliest first (ties broken by course name, then course id), and each course is placed into the free time left after the courses before it — so study sessions never overlap. Feasibility, emergency mode and the infeasibility warning (edge cases 1 and 3) are judged per course, on the time that course actually has left. See [ADR 0011](docs/adr/0011-one-plan-per-student-nearest-exam-first.md).
...
### 6. Calendar
- **FR6.1** — A Calendar view renders the schedule already produced by FR3.1–FR3.3 in a real weekly time grid (Sunday–Saturday, the same week as the sprint in FR4.0), with an optional month view. Each event is a **topic**: a topic's consecutive scheduled learning actions appear as one event covering their combined time (the review session and study-aid preparation are events of their own), and clicking an event opens that topic's detail view — the same one the board uses — with the event's subtasks highlighted. By default the Calendar shows **all courses together** — the one combined plan of FR3.1, with a course label on each session — and can be filtered to a single course, which shows that course's slice of the same plan at the same times. The summary metrics (free time, planned time, sessions) reflect whichever courses are shown. This is a visualization of existing schedule output — no new scheduling logic. It replaces the earlier Gantt-style list; there is no separate Gantt view.
- **FR6.2** — Optional Google Calendar sync. The LearnSprint Calendar is the primary schedule; a student may additionally connect their Google account (a separate consent from Google sign-in — see [ADR 0010](docs/adr/0010-google-calendar-sync-via-direct-api.md)) and sync the current plan into a dedicated "LearnSprint" calendar the app creates in their Google account. Sync is manual ("Sync now"), follows the Calendar's course filter (all courses, or the one selected), and **replaces each synced course's sessions** in that calendar with the current plan, so syncing again never duplicates and courses not being synced are left alone; disconnecting removes the calendar and the stored credential. The `.ics` download follows the same filter. The app never writes to the student's own calendars. The `.ics` download stays available as the no-account alternative.
...
| **Action** | `id`, `topicId`, `type: 'read' \| 'summarize' \| 'quiz' \| 'custom'`, `title`, `order`, `defaultDurationMinutes` | Shared (default estimate, not the actual scheduled time) |
| **Material** | `id`, `userId`, `courseId`, `topicId?` (null while analysed but not yet filed, and for a syllabus), `fileName`, `s3Key`, `fileType`, `sizeBytes`, `uploadedAt`, `analysis?` (what the AI or the heuristic understood, kept for confirmation), `confirmedAt?` | Private (owned by the uploading student; FR2.8–FR2.10) |
