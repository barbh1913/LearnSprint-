...
- **FR6.2** — Optional Google Calendar sync. The LearnSprint Calendar is the primary schedule; a student may additionally connect their Google account (a separate consent from Google sign-in — see [ADR 0010](docs/adr/0010-google-calendar-sync-via-direct-api.md)) and sync the current plan into a dedicated "LearnSprint" calendar the app creates in their Google account. Sync is manual ("Sync now") and **replaces that course's sessions** in that calendar with the current plan, so syncing again never duplicates and other courses' sessions in the same calendar are left alone; disconnecting removes the calendar and the stored credential. The app never writes to the student's own calendars. The `.ics` download stays available as the no-account alternative.
