...
const courseSchedule = (course: typeof dataStructures, blocks: (typeof treesBlock)[]) => ({
  courseId: course.id,
  ...
  totalNeededMinutes: 60 * blocks.length,
  blocks,
})

const plan = {
  courses: [courseSchedule(dataStructures, [treesBlock]), courseSchedule(oop, [classesBlock])],
  blocks: [treesBlock, classesBlock],
...
describe('CalendarPage', () => {
  it('opens on all courses, labelling each session with its course', async () => {
    ...
    expect(await screen.findByRole('button', { name: /Read: Trees, Data Structures/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Read: Classes, OOP/ })).toBeInTheDocument()
  ...
  it('filters to one course as a slice of the same plan, without the course label', async () => {
    ...
    renderPage()
    await screen.findByRole('button', { name: /Read: Classes, OOP/ })
    ...
    expect(screen.getByRole('button', { name: 'Read: Trees, 06:00 PM–07:00 PM' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Read: Classes/ })).not.toBeInTheDocument()
  ...
  it('explains that there is no plan when no course has an exam date', async () => {
    mockApi({
      ...defaultResponses,
      '/schedule': { courses: [], blocks: [], totalAvailableMinutes: 0, totalNeededMinutes: 0, sessions: 0 },
    })
  ...
  it('opens the topic behind an event with that action highlighted', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /Read: Classes/ }))
