...
- **One LearnSprint-owned calendar per student.** On connect, the app creates a secondary calendar named "LearnSprint" in the student's account and stores its id. Every sync **replaces** that calendar's events with the current plan — clear, then insert. Disconnect deletes the calendar. This is what makes re-syncing idempotent without any per-event bookkeeping, and it is the guarantee that the app never touches the student's own calendars. It also lets the consent ask for the narrowest scope Google offers (`calendar.app.created`, which covers only calendars the app created) rather than blanket access to all events.
...
- **Store Google event ids and diff on each sync.** More "correct" in the abstract, but it introduces exactly the stored-and-can-drift state ADR 0005 avoids for the schedule itself. Replace-everything is dumber and therefore never wrong.
...
- **A sync is a few batched HTTP calls, not one per block**, so it fits comfortably in the synchronous request the rest of the app uses. The Lambda's timeout was not re-verified for this change and should be checked against real sync durations once the feature is live.
