...
- **Deploying an update is still a manual sequence of AWS CLI calls**, not a script or a CI job — see the redeploy steps in [docs/deployment-setup.md](../deployment-setup.md). `deploy-frontend.yml` exists but needs its GitHub secrets set and an OIDC role scoped to this repository (the existing `githubactions-s3-fullaccess` role is scoped to two other repositories and was deliberately not widened to include this one).
